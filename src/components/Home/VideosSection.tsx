...
export function VideosSection() {
    ...
    return (
        <section className="py-8 sm:py-10 md:py-12 lg:py-[60px] xl:py-[76px]">
            ...
            {/* Сетка видео */}
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-5 md:gap-5.5 lg:gap-6 xl:gap-8">
                {featuredVideos.slice(0, 2).map((video) => {
                    ...
                    return (
                        ...
                        >
                            {/* Карточка видео */}
                            <div className="relative w-full h-[253px] sm:h-[296px] md:h-[350px] lg:h-[425px] xl:h-[470px] rounded-[16px] sm:rounded-[20px] md:rounded-[22px] lg:rounded-[24px] overflow-hidden">
                                {/* Видео превью */}
                                {video.videoUrl ? (
                                    <video
                                        src={video.videoUrl}
                                ...
                                <div className="absolute top-3 sm:top-3.5 md:top-4 xl:top-5 left-3 sm:left-3.5 md:left-4 xl:left-5 z-20">
                                    <span className="text-[12px] sm:text-[14px] md:text-[16px] xl:text-[18px] font-montserrat font-normal text-white drop-shadow-lg">
                                        /{String(videoNumber).padStart(2, '0')}
                                    </span>
                                </div>

                                {/* Длительность (справа вверху) */}
                                <div className="absolute top-3 sm:top-3.5 md:top-4 xl:top-5 right-3 sm:right-3.5 md:right-4 xl:right-5 bg-white/90 backdrop-blur-sm rounded-full px-2 sm:px-2.5 md:px-3 py-0.5 sm:py-0.75 md:py-1 z-20">
                                    <span className="text-[10px] sm:text-[11px] md:text-[12px] xl:text-[14px] font-montserrat font-medium text-black">
                                ...
                                <div className="absolute bottom-4 sm:bottom-5 md:bottom-6 xl:bottom-8 right-4 sm:right-5 md:right-6 xl:right-8 w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 xl:w-14 xl:h-14 rounded-full border-1 border-white flex items-center justify-center z-20">
                                    <svg className="w-5 h-5 sm:w-5.5 sm:h-5.5 md:w-6 md:h-6 xl:w-7 xl:h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {featuredVideos.slice(2, 3).map((video) => {
                    ...
                    return (
                        ...
                        >
                            {/* Карточка видео */}
                            <div className="relative w-full h-[253px] sm:h-[296px] md:h-[382px] lg:h-[425px] xl:h-[470px] rounded-[16px] sm:rounded-[20px] md:rounded-[22px] lg:rounded-[24px] overflow-hidden">
                                {/* Видео превью */}
                                {video.videoUrl ? (
                                    <video
                                        src={video.videoUrl}
